...
const   ChapterContainer = async ({ id ,isSheet}: { id: string,isSheet:boolean }) => {
  console.log(id,"kk");
        ...
        <AccordionItem value="item-1" className="w-full   ">
          <AccordionTrigger className="flex  w-full  p-3 h-32 ">
            <div className=" overflow-hidden  aspect-square  flex h-full relative ">
              <img
                src={premiumChapter.length>0?(premiumChapter[0].thumbnail):("")}
                alt=""
          ...
          <AccordionContent>
            <ScrollArea className="border-l-4 border-blue-600">
              {premiumChapter.length>0 && premiumChapter.map((e) => (
                <>
                  <Link
                    key={e.id}
                    href={`/chapter/${e.id}?n=${e.series}`}
...
{freeChapter.length>0 &&(
      ...
      </div>):(<>No novel with this f***ing id</>)}
    </div>
  );
};
